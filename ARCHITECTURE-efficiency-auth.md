...
## 4. 落地实现顺序（进度）
1. ✅ `public/era.css` + `public/runtime.js`
2. ✅ 壳 `lib/shell.mjs`（生成只填 `<main>`；接 Next 时转 layout.tsx）
3. ✅ `lib/macros.mjs` + `lib/blocks.mjs`：宏展开 + 站点/广告池
4. ✅ prompt 三件套 `prompts/{output-contract,aesthetic,era}.md` + `lib/prompt.mjs`（缓存友好排序 + max_tokens + 防注入）
5. ✅ 后端编排 `lib/{kv,llm,auth,generate}.mjs`：JWT 校验 + 每日配额 + 计量 + `setIfAbsent`（全带 mock 降级，`demo/generate-demo.mjs` 验证）
6. ✅ 静态落地页 `public/landing.html` + 登录 `public/login.html`（Supabase 魔法链接，mock 可跑）
7. ✅ 沙箱 iframe 宿主 `public/app.html`（红线一：`sandbox` 无 `allow-same-origin` + postMessage 内链）

8. ✅ Next.js 脚手架：`app/` 路由 + `app/api/{generate,stats,config}/route.js`，`lib/*.mjs` 全部接入并实跑联调
9. ✅ 真实环境实现（全部带 mock 优雅降级）：
   - `lib/kv.mjs`：有 `KV_REST_API_URL` 走 `@vercel/kv`（含 nx 写入 + 配额 TTL），否则 globalThis 内存
   - `lib/llm.mjs`：有 `DEEPSEEK_API_KEY` 走 DeepSeek，否则吐样张；`generate.mjs` 加 `extractFragment` 清洗真实模型的 ```html 围栏/外壳
   - `lib/auth.mjs` + `/api/config` + `login/app.html`：有 Supabase 凭据走魔法链接登录 + session token，否则 `mock:<uid>`
   - `generate.mjs` 计量：配 `SUPABASE_SERVICE_ROLE_KEY` 时异步写 `generations` 表
   - `.env.local.example` + `supabase/schema.sql` 就位

### 接真实环境 · 一键开关（填 `.env.local` 即生效）
```
1. cp .env.local.example .env.local
2. DeepSeek：填 DEEPSEEK_API_KEY                          → 关闭 mock LLM
3. Vercel KV：建库后 vercel env pull，填 KV_REST_API_*     → 内存换持久化
4. Supabase：填 URL/ANON/SERVICE_ROLE_KEY；SQL Editor 跑 supabase/schema.sql
   控制台 Auth 开启 Email magic link，把站点 URL 加入 Redirect URLs（含 /app.html）
5. 重启 npm run dev（Windows 加 NEXT_TELEMETRY_DISABLED=1）
验证：/api/config 应返回 authMode:"supabase"
```

### 仍待办
- [ ] DeepSeek **流式**边收边渲染（当前整段返回再落库；流式需配合 iframe 渐进写入）
- [ ] 全局成本熔断（当日总花费阈值，读 generations 聚合）
- [ ] `/api/feedback` + 反馈页接 `supabase/schema.sql` 的 feedback 表
- [ ] `next build` 生产构建校验 outputFileTracing 是否带上 prompts/
