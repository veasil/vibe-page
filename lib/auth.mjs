...
export async function requireUser(authHeader){
  ...
  if (!hasSupabase){
    return mockUserFromToken(token);
  }
...
// mock token 解析（与 hasSupabase 无关）；admin 强制 mock 时即便配了 Supabase 也用它
function mockUserFromToken(token){
  const m = /^mock:(.+)$/.exec(token);
  if (!m) throw new AuthError("mock 模式 token 需形如 mock:<uid>");
  return { id: m[1], email: `${m[1]}@local`, mock: true };
}

/** 强制 mock 鉴权：忽略 env，只认 "mock:<uid>" Bearer（admin 开关 + dev 用） */
export function requireMockUser(authHeader){
  const token = (authHeader || "").replace(/^Bearer\s+/i, "").trim();
  if (!token) throw new AuthError("未登录");
  return mockUserFromToken(token);
}
