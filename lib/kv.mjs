...
// ——— 跨浏览器登录中转（relay）：存 session 5 分钟，一次性取走 ———
export async function setRelay(code, data) {
  const key = `relay:${code}`;
  if (hasKV) {
    await (await vkv()).set(key, JSON.stringify(data), { ex: 300 });
  } else {
    _mem.set(key, { data, exp: Date.now() + 300_000 });
  }
}
export async function takeRelay(code) {
  const key = `relay:${code}`;
  if (hasKV) {
    const kv = await vkv();
