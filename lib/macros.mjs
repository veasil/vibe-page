...
// 站点/词条名 → 绝对路径生成型内链（点击生成该词条的 era 页面；绝对路径 = 全站唯一、可被自引用）
const slugAttr = name => `class="inner-link" data-slug="/${esc(name)}"`;

function renderQuickGrid(){
  const lis = QUICK_APPS.map(t =>
    `<li><a ${slugAttr(t)}><span class="ico" style="background:${colorOf(t)}">${esc(t.charAt(0))}</span>${esc(t)}</a></li>`
  ).join("");
...
function renderLinks(cat){
  ...
  const lis = s.links.map(([name, flag]) => {
    ...
    return `<li><a class="inner-link ${cls}" data-slug="/${esc(name)}">${favSpan(name)}<span class="txt">${esc(name)}</span></a></li>`;
  }).join("");
  const subs = s.sub.map(x => `<a ${slugAttr(x)}>${esc(x)}</a>`).join("");
  return `<div class="panel"><div class="panel-hd">`
    + `<span class="bar" style="background:${s.color}"></span><h3>${esc(cat)}</h3>`
    + `<span class="sub">${subs}</span><a class="inner-link more" data-slug="/${esc(cat)}">更多 &gt;&gt;</a></div>`
...
function renderHotRank(){
  ...
  const lis = HOT_RANK.map((t, i) =>
    `<li><span class="n">${i+1}</span><a class="inner-link" data-slug="/search/${esc(t)}">${esc(t)}</a>${i < 2 ? ' <span class="fire">热</span>' : ''}</li>`
  ).join("");
