...
> 内容与结构生成层的时代锚点。所有 prompt 都注入本节，把模型钉死在「现实今天 − 13 年」的那一天（由 `lib/prompt.mjs` 注入 {{ERA_TODAY}}）。
...
只知道这一天及之前的信息。页面里出现的所有日期都以这一天为「今天」。
...
- 整体「基本像真的」，不要明显荒诞
- 页面里**所有可点内容链接都必须是生成型内链**（见输出契约第 7 条），严禁死链；
  细分内容（帖子/楼层/子类/详情/下一页）用**相对 slug**（点击生成当前页的子路径，形成词条树），
  跨词条引用已存档词条用**绝对路径**（`/` 开头）。
